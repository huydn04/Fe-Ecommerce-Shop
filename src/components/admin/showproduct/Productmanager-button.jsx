...
const Popup = () => {
    ...
    return(
        <div className='h-[50px] rounded-md flex justify-around absolute ml-8 bottom-0 '>
                ...
                <button onClick={() => setShowDelete(true)} className='w-[100px]  bg-[#d0cccd] text-xl flex justify-center rounded-md items-center hover:bg-[#f74a5d]'><RiDeleteBin6Line className='mr-3'/> Xoá</button>

                
                <ModalUpdate isVisible={showUpdate} onClose={() => setShowUpdate(false)}/>

                <ModalDelete isVisible={showDelete} onClose={() => setShowDelete(false)}/>
        </div>
    )
}
